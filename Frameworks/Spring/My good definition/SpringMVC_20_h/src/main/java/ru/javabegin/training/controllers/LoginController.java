...
public class LoginController {
    ...
    private static final int FAIR_PASSWORD = 5;
    private static final int STRONG_PASSWORD = 7;
    ...
    public String checkPassword(@RequestParam String password){
	    ...
	    if(password.length() >= WEAK_PASSWORD && password.length() < FAIR_PASSWORD){
	        return "Слабый";
        }
        else if(password.length() >= FAIR_PASSWORD && password.length() < STRONG_PASSWORD){
            return "средний";
        }
        else if(password.length() > STRONG_PASSWORD){
            return "сильный";
        }
